...
// Next.js 15 uyumlu async Metadata
export async function generateMetadata({ params }: { params: any }) {
  const resolvedParams = await params; // Parametreyi asenkron olarak bekle
...
// Next.js 15 uyumlu async Page Component
export default async function BlogPostPage({ params }: { params: any }) {
  
  // 1. URL'den gelen parametreyi çöz ve bekle
  const resolvedParams = await params;
  const decodedSlug = decodeURIComponent(resolvedParams.slug).trim();
  
  // 2. JSON'daki verilerle (boşlukları temizleyerek) tam eşleştirme yap
  const post = postsData.find((p) => p.slug.trim() === decodedSlug);

  // 3. Eşleşme başarısızsa terminale sorunu yazdır ve 404'e at
  if (!post) {
